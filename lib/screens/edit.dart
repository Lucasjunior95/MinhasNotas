...
class _EditNotePageState extends State<EditNotePage> {
                ...
                child: SafeArea(
                  child: Row(
                    children: <Widget>[
                      ...
                      Spacer(),
                      IconButton(
                        tooltip: 'Marcar nota como importante',
  ...
  Future<Type> handleSave() async {
    setState(() {
      currentNote.title = titleController.text;
      currentNote.content = contentController.text;
      print('Olá ${currentNote.content}');
    });

    if (isNoteNew) {
      var latestNote = await NotasDatabase.db.addNoteInDB(currentNote);
      setState(() {
        currentNote = latestNote;
        return MyHomePage;
      });
    } else {
      await NotasDatabase.db.updateNoteInDB(currentNote);
    }
    setState(() {
      isNoteNew = false;
      isDirty = false;
      ...
      showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content: Text('Esta nota será excluída permanentemente'),
              actions: <Widget>[
                FlatButton(
                  child: Text('DELETAR',
                      style: prefix0.TextStyle(
                          ...
                          letterSpacing: 1)),
                  onPressed: () async {
                    await NotasDatabase.db.deleteNoteInDB(currentNote);
                    widget.triggerRefetch();
                    Navigator.pop(context);
                    Navigator.pop(context);
                  },
